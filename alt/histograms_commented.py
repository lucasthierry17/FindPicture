...
def load_and_convert_image(image_path):
    """
    Loads and converts an image from BGR to RGB using OpenCV.
    """
    image = cv2.imread(image_path)
...
def display_images(input_image_path, similar_images):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        futures = [executor.submit(load_and_convert_image, input_image_path)]
        
        # Load similar images
        for image_path, _ in similar_images:
            futures.append(executor.submit(load_and_convert_image, image_path))
        
        # Retrieve all images after they are loaded
    ...
    # Plot similar images
    for i, (image, (_, distance)) in enumerate(zip(images[1:], similar_images), start=2):
        plt.subplot(1, 6, i)
        plt.imshow(image)
